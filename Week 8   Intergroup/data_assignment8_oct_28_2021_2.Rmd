...
CJ <- ChinaJapan %>% 
  mutate(j_mean = rowMeans(select(., 3:6))) %>% 
  mutate(c_mean = rowMeans(select(., 7:10))) %>% 
  mutate(diff = (c_mean - j_mean))
  
CJ$avg_j <- mean(CJ$j_mean)
CJ$avg_c <- mean(CJ$c_mean)
CJ$avg_diff <- mean(CJ$diff)

# base R way to get row means:  CJ$diff_mean <- apply(CJ[,27:],1,mean)
