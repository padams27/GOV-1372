...
ggplot(data = ggl, mapping = aes(x = age)) + # mapping = aes() controls which 
  ...
  geom_histogram() + # geoms, like geom_histogram, control the type of plot you are making
  labs(x = "Age", y = "Count")

```

The distribution shows that voter turn-out in younger populations is far lower than they should be and the overall proportion of voting per age group is higher in those who are older.
...
mean(ggl$voted[ggl$treatment == "Control"]) 
mean(ggl$voted[ggl$treatment == "Civic Duty"]) 
mean(ggl$voted[ggl$treatment == "Hawthorne"]) 
mean(ggl$voted[ggl$treatment == "Self"]) 
mean(ggl$voted[ggl$treatment == "Neighbors"]) 


```

The results show that the proportion of people who vote increases as more social pressure is applied.
...
treatment_model <- ggl %>% 
  ...
  mutate(term = c("Intercept", "Control", "Hawthrone", "Neighbors", "Self")) %>% 
  gt() %>% 
  tab_header(title = "Effect of the treatment on vote turn out") %>% 
  cols_label(term = "",
...
This table helps backup my answer to the previous question even though it isn't a new analysis.
...
sex_model <- ggl %>% 
  ...
  tidy(conf.int = T) %>% 
  select(-c(std.error, p.value, statistic)) %>% 
  mutate(term = c("Intercept", "Female")) %>% 
  gt() %>% 
  tab_header(title = "Effect of sex on vote turn out") %>% 
  cols_label(term = "",
             estimate = "Coefficient",
             conf.low = "5th percentile",
             conf.high = "95th percentile") %>%
...
This model shows the difference between male and female voter turn-out. The interpretation of the coefficient value is that women are 0.01 times more likely to vote than men. This is a very small difference but something to note nonetheless.
...
sex_treat_model <- ggl %>% 
  lm(voted ~ sex * treatment, .) %>% 
  tidy(conf.int = T) %>% 
  select(-c(std.error, p.value, statistic)) %>% 
  gt() %>% 
  tab_header(title = "Effect of sex on treatment effect via vote turn out") %>% 
  cols_label(term = "",
             estimate = "Coefficient",
             conf.low = "5th percentile",
             conf.high = "95th percentile") %>%
