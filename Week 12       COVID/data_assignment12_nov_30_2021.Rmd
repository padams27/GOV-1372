...
covid_data %>% 
  summarise_at(c(11,12), mean, na.rm = TRUE) %>% 
  mutate(percent_agree = (list_treatment - list_control) * 100)
...
covid_data %>% 
  summarise(median(covid_burden, na.rm = TRUE))

covid1 <- covid_data %>% 
  mutate(relative_burden = case_when(covid_burden >= 3.7 ~ TRUE,
                                     covid_burden < 3.7 ~ FALSE))

covid1 %>% 
  group_by(relative_burden) %>% 
  summarise_at(c(11,12), mean, na.rm = TRUE) %>% 
  mutate(percent_agree = (list_treatment - list_control) * 100) %>% 
  select(1, 4)
...
covid_data %>% 
  ...
  theme_classic() +
  labs(title = "Perceived COVID Burden by Counrty",
       x = "COVID Burden",
       y = "Density",
       fill = "Country")
...
covid_data %>% 
  ggplot(aes(x = covid_burden, y = radical_intention, colour = country)) +
  geom_smooth(method = 'lm', se = FALSE) +
  theme_classic() +
  scale_color_brewer(palette = 'PRGn') +
  labs(title = "Regression of COVID Burden and Radical Intention by Counrty",
       x = "COVID Burden",
       y = "Radical Intention",
       colour = "Country")
...
model <- lm(covid_burden ~ radical_intention + age + gender + education, data = covid1)
...
model2 <- covid1 %>% filter(country != "United States") %>% lm(covid_burden ~ radical_intention + age + gender + education + spectrum, .)
